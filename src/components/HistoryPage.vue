<template>
    ...
    <div v-if="videosStore.length === 0" class="flex flex-col items-center justify-center py-20">
        ...
        <router-link v-if="!getPreferenceBoolean('watchHistory', true)" to="/user/gust/preferences" class="btn">{{ $t('actions.enable_watch_history') || 'Enable Watch History' }}</router-link>
    </div>
