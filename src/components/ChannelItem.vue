<template>
    <div
        class="h-full flex flex-col items-center justify-between card p-4 text-center transition-all duration-200 hover:shadow-lg"
    >
        <!-- Channel Info -->
        <router-link :to="item.url" class="flex flex-col items-center gap-3">
            ...
            <div class="relative">
                <img
                    loading="lazy"
                    :src="item.thumbnail"
                    class="h-24 w-24 rounded-full bg-surface object-cover"
                    alt=""
                />
                <div
                    v-if="item.verified"
            ...
            <!-- Name -->
            <div>
                <h3 class="text-main font-semibold" :title="item.name">{{ item.name }}</h3>
